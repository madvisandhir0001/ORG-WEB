    ...
    const handleClick = e => {
        ...
        if (name.length >= 3 && email.length > 0 && phoneNo.length === 10 && password.length >= 6 && confirmPassword.length >= 0) {
            if (confirmPassword == password) {
                console.log("Runned")
                setProgress(true)
                createUserWithEmailAndPassword(email, password).then(() => setProgress(false))
            } else {
                setError2({ type: '7', message: 'Password not Match' })
            }
        } else {
            password.length < 6 && setError2({ type: '5', message: 'Password must  6 characters' })
        ...
        <Container>
            <LoginContainer>
                ...
                <Form>
                    ...
                    {/* {error2 && <Error>{error2.message}</Error>} */}
                    <InputField
                        id="outlined-search"
                        variant="outlined"
                        value={name}
